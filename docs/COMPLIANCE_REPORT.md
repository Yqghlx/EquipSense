...
| 多因素认证（MFA） | TOTP 设置/确认/禁用 + Redis 一次性登录挑战；生产强制 SystemAdmin/MaintenanceLead 首次注册 | ✅ 已实现 |
...
- [x] **MFA 强制策略** — 生产配置强制 SystemAdmin/MaintenanceLead 完成 TOTP enrollment；未完成前不颁发 JWT，刷新令牌和禁用操作也会被拦截
...
EquipSense 已实现等保 2.0 三级要求的核心安全控制（身份鉴别/访问控制/安全审计/通信保密/完整性保护），并提供高权限 TOTP enrollment 与数据库/附件备份能力。剩余差距是敏感字段加密、MFA 恢复流程、备份定时化和恢复演练；这些必须在正式生产验收中完成。

**建议**：
1. 生产部署前验证高权限账号完成 MFA enrollment，并演练受控恢复流程
