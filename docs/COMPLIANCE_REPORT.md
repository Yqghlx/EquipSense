...
| 数据备份 | 未实现（依赖运维层 PostgreSQL 备份策略） |  待实现 |
...
- [ ] **多因素认证（MFA）** — 核心系统建议启用，目前仅密码
- [ ] **敏感字段加密** — 除密码外，手机号/邮箱等 PII 字段未加密存储
- [ ] **数据备份策略** — 应用层未实现，需运维层配置 PostgreSQL 定时备份
...
EquipSense 已实现等保 2.0 三级要求的核心安全控制（身份鉴别/访问控制/安全审计/通信保密/完整性保护）。剩余差距（MFA/敏感字段加密/数据备份）属于运维层或 Phase 3+ 规划项，不影响当前版本上线。

**建议**：
1. 生产部署前完成 MFA 启用
2. 配置 PostgreSQL 定时备份（pg_dump + cron）
3. 定期更新 WAF 规则库
