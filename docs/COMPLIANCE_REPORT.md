...
EquipSense 已实现等保 2.0 三级要求的核心安全控制（身份鉴别/访问控制/安全审计/通信保密/完整性保护），并提供高权限 TOTP enrollment、一次性 MFA 恢复码与数据库/附件备份能力。剩余差距是敏感字段加密、备份定时化，以及 MFA/备份恢复演练；这些必须在正式生产验收中完成。
