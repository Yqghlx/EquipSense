...
> **当前状态说明（2026-08-12）**：本文保留历史功能盘点作为基线；当前质量门禁以仓库实际测试结果为准。最新验证包含事务 Outbox/Inbox、自动建单事务回滚、1344 个后端单元测试、默认集成测试 160 通过/6 跳过，以及显式启用 RabbitMQ 后 166/166 通过、0 跳过、0 失败，前端 416 个单元测试、真实 PostgreSQL/RabbitMQ 迁移核验、关键流程干净浏览器冒烟、后端/前端/边缘网关生产构建、备份恢复真实实演，以及最近一次提交三镜像的隔离 Production runtime smoke（431 个 E2E 通过、2 个条件跳过、0 失败）。本轮同时修复网关设备管理入口、四步接入向导验收、工单 UI 假绿断言，完成网关管理、网关设备、网关监控、通知中心、待审批页面、应用壳层反馈、审批链配置、外部集成配置、通知偏好设置、知识库来源、候选规则审核、仪表盘工单状态、工单详情、登录 MFA、用户管理角色下拉、设备详情网关选择、OPC UA/Modbus 连接表单、系统信息运行时、订阅方案、租户账单/时区、审计动作筛选、工单审批与离线徽章的中英文覆盖与回归测试；同时补齐 Redis 分布式锁租约续期：长后台任务按 token 原子 PEXPIRE，续租失败取消当前任务，正常结束立即释放锁，并增加 5 个后端回归用例；修复两个会导致 E2E 假绿的条件跳过（强制改密使用正确用户管理路由，通知中心首屏失败改为硬断言）；新增 `docker/compose-production.sh`：启动类操作 fail-closed，配置不完整时 `ps/logs/exec/stop/down` 使用仅含安全变量白名单且未知变量默认丢弃的恢复环境仍可诊断和处置；运行时门禁同时拒绝 644 私钥、Mosquitto 密码文件和相关敏感文件符号链接，避免部分容器上线后无法收集状态、误写认证目标或泄露认证材料。隔离备份恢复实演已验证 PostgreSQL/TimescaleDB、附件和恢复后健康检查；本轮修正后的 E2E 已通过 TypeScript 与 Playwright 列表编译检查，但尚未在本机重新启动三镜像运行时。当前 `docker/.env` 有 24 个配置问题，连同运行时 TLS/MQTT 文件检查共报告 27 个发布门禁问题，修复前不得上线。
...
| 前端单元测试 | ✅ 416/416 通过 |
| E2E 测试 | ⚠️ 最近一次开发栈与隔离 Production 镜像证据为 433 个场景：431 通过、2 跳过、0 失败；本轮已修正两个条件跳过并通过 E2E TypeScript/列表检查，需在隔离三镜像重新运行后更新运行统计；main/tag 已接入 Production 全量门禁 |
