...
> **FMEA 关联选择器增量（2026-08-13）**：FMEA 表单现在通过只读规则摘要接口按设备类型选择知识规则，不再要求用户手工复制 UUID；系统预置规则可直接关联，编辑停用规则会保留当前关系，查询失败时仍允许保存未关联条目。接口只返回规则 ID、名称、设备类型、启用状态和系统预置标识，服务端继续执行最终租户校验；新增 FMEA 服务/控制器租户隔离测试、前端 Hook/选择/清除回归和双语 i18n 检查；当前前端全量门禁为 86 个测试文件、494/494 通过，i18n 为 1110 个键完全对齐。本增量不改变当前真实生产凭据、证书、WAF、SMTP、OTLP、现场协议、容量基线和 27 项部署门禁的未完成状态。
...
| 前端页面 | 30 个 |
| 单元测试 | 1692/1692 个（后端）+ 494/494 个（前端） |
| 集成测试 | 193 个用例（36 个文件） |
...
| 后端单元测试 | ✅ 全量 1692/1692 通过；审批权限聚焦 17/17、异常映射 18/18；边缘网关生命周期聚焦 18/18、健康端点聚焦 7/7、断网缓冲并发聚焦 33/33、CloudUploader 回放聚焦 13/13、网关代理取消传播聚焦 11/11 通过；告警通知/邮件聚焦 31/31 通过；运营报表聚焦 10/10 通过；规则/FMEA 诊断聚焦 21/21 通过 |
| 后端集成测试 | ✅ 默认 193 总数：187 通过、6 条件跳过、0 失败；邮件告警事务聚焦通过；审批 HTTP 授权集成 7/7 通过 |
| 前端代码质量 | ✅ 0 TODO/FIXME/console.log |
| 前端类型检查 | ✅ 0 错误（TypeScript strict） |
| 前端单元测试 | ✅ 86 个测试文件、494/494 通过 |
| E2E 测试 | ✅ 当前 Production 三镜像完整 435 项验收为 434 通过、1 个架构性条件跳过、0 失败（2 workers，16.3 分钟） |
| i18n 完整性 | ✅ 1110 个键在中英文资源中完全对齐 |
