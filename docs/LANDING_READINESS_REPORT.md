...
> **当前状态说明（2026-08-12）**：本文保留历史功能盘点作为基线；当前质量门禁以仓库实际测试结果为准。最新验证包含事务 Outbox/Inbox、自动建单事务回滚、1397 个后端单元测试、默认集成测试 161 通过/6 跳过，以及显式启用 RabbitMQ 后 167/167 通过、0 跳过、0 失败，前端 424 个单元测试、真实 PostgreSQL/RabbitMQ 迁移核验、关键流程干净浏览器冒烟、后端/前端/边缘网关生产构建、备份恢复真实实演，以及最新三镜像隔离 Production runtime smoke（433 个 E2E 中 432 个通过、1 个架构性条件跳过、0 失败）。本轮同时补上后端强制改密门禁：JWT 携带 `must_change_password`，业务 API 统一返回 403，改密成功吊销旧 refresh 会话并签发新会话；MFA 管理接口不会绕过该门禁；修复强制改密登录后的路由卸载问题、通知中心首屏假绿断言和并发多标签页会话恢复断言；修复注册、设备和工单表单字段级错误未关联到输入框的无障碍与反馈问题，并修正设备/工单下拉框必填校验的中英文提示；新增生产种子账户、边缘网关设备来源和 OPC UA 安全配置门禁，禁止普通 Production 启用第二租户测试管理员，禁止后端不可达时回退到镜像内开发示例设备，并默认拒绝实际 OPC UA 设备使用明文或未知安全模式；动态配置刷新同样复用 OPC UA 门禁，不安全批次不会应用，成功空列表会停止已删除设备，后端暂时不可达会保留当前采集配置；修复 SLA/遥测后台任务吞掉停机取消信号的问题，并让 InMemory 应急事件总线在停机时立即停止后续处理器，避免容器停机时继续产生副作用；新增 `docker/compose-production.sh`：启动类操作 fail-closed，配置不完整时 `ps/logs/exec/stop/down` 使用仅含安全变量白名单且未知变量默认丢弃的恢复环境仍可诊断和处置；运行时门禁同时拒绝 644 私钥、Mosquitto 密码文件和相关敏感文件符号链接，避免部分容器上线后无法收集状态、误写认证目标或泄露认证材料。隔离备份恢复实演已验证 PostgreSQL/TimescaleDB、附件和恢复后健康检查。新增 OEE 查询、LLM 调用和 MQTT 后台服务的取消/超时边界回归，确保停机取消不被伪装成业务降级或错误日志。当前 `docker/.env` 有 24 个配置问题，连同运行时 TLS/MQTT 文件检查共报告 27 个发布门禁问题，修复前不得上线。

> 本轮新增验证：根因分析、知识沉淀、工单外部集成和告警通知的普通故障降级不会吞掉宿主停机/处理超时取消，取消会继续传播给消息总线；OEE 遥测查询与 LLM 调用区分主动取消和内部超时；设备离线与网关心跳监控采用条件更新，避免状态快照之后刚恢复通信的对象被误标记离线或误发通知；RabbitMQ 启动阶段收到宿主取消时不再记录严重启动故障，正常停机连接关闭记录为信息日志，停机取消不会污染 Inbox 失败指标或失败状态；注册、设备和工单表单校验错误现在通过 `aria-invalid`、`aria-describedby` 和告警语义准确关联输入框，下拉框必填校验也复用中英文业务提示；Release 构建、RabbitMQ 167/167 集成、生产脚本和生产 runtime smoke 均已复验通过。
