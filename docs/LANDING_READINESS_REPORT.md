...
> **当前状态说明（2026-08-12）**：本文保留历史功能盘点作为基线；当前质量门禁以仓库实际测试结果为准。最新验证包含事务 Outbox/Inbox、自动建单事务回滚、1461 个后端单元测试、默认集成测试 163 通过/6 跳过，以及既有显式 RabbitMQ 集成门禁 167/167 通过、0 跳过、0 失败，前端 431 个单元测试、真实 PostgreSQL/RabbitMQ 迁移核验、关键流程干净浏览器冒烟、后端/前端/边缘网关生产构建、备份恢复真实实演，以及最新三镜像隔离 Production runtime smoke（433 个 E2E 中 432 个通过、1 个架构性条件跳过、0 失败）。本轮同时补上后端强制改密门禁：JWT 携带 `must_change_password`，业务 API 统一返回 403，改密成功吊销旧 refresh 会话并签发新会话；MFA 管理接口不会绕过该门禁；修复强制改密登录后的路由卸载问题、通知中心首屏假绿断言和并发多标签页会话恢复断言；修复注册、设备、工单、登录/MFA、密码恢复和用户管理表单字段级错误未关联到输入框的无障碍与反馈问题，并修正设备/工单下拉框必填校验的中英文提示；新增生产种子账户、边缘网关设备来源和 OPC UA 安全配置门禁，禁止普通 Production 启用第二租户测试管理员，禁止后端不可达时回退到镜像内开发示例设备，并默认拒绝实际 OPC UA 设备使用明文或未知安全模式；动态配置刷新同样复用 OPC UA 门禁，不安全批次不会应用，成功空列表会停止已删除设备，后端暂时不可达会保留当前采集配置；修复 SLA/遥测后台任务吞掉停机取消信号的问题，并让 InMemory 应急事件总线在停机时立即停止后续处理器，避免容器停机时继续产生副作用；新增 `docker/compose-production.sh`：启动类操作 fail-closed，配置不完整时 `ps/logs/exec/stop/down` 使用仅含显式安全变量白名单且未知变量默认丢弃的恢复环境仍可诊断和处置；运行时门禁同时拒绝 644 私钥、Mosquitto 密码文件和相关敏感文件符号链接，避免部分容器上线后无法收集状态、误写认证目标或泄露认证材料。隔离备份恢复实演已验证 PostgreSQL/TimescaleDB、附件和恢复后健康检查。新增 OEE 查询、LLM 调用和 MQTT 后台服务的取消/超时边界回归，确保停机取消不被伪装成业务降级或错误日志。本轮进一步收口通知可靠性：告警触发不再由实时推送服务重复写入 `Guid.Empty` 孤儿通知，解除告警、SLA 升级、设备离线和网关离线改为按租户内活动运维角色展开站内通知，停用账号不再收到新通知；取消令牌贯穿 SignalR 接口、事件处理器和后台监控，取消不会被普通故障隔离逻辑吞掉；通知中心新增 SignalR/Web Push 用户偏好筛选、租户内用户组定向和批量 Push 投递，Email 通道明确保持未开放；同时修复 JWT 仅携带标准 `sub` 声明时当前用户 ID 未解析导致通知历史为空的问题。当前 `docker/.env` 有 24 个配置问题，连同运行时 TLS/MQTT 文件检查共报告 27 个发布门禁问题，修复前不得上线。
...
| 后端单元测试 | ✅ 1461/1461 通过 |
| 后端集成测试 | ✅ 默认 163 通过、6 条件跳过；既有 RabbitMQ 门禁显式设置 `RUN_RABBITMQ_INTEGRATION_TESTS=true` 时 167/167 通过、0 跳过、0 失败 |
...
| i18n 完整性 | ✅ 1003 个键在中英文资源中完全对齐 |
| 生产构建 | ✅ PWA SW 产出 + precache 128 entries + 边缘网关镜像可复现构建 |
| 生产配置 | ✅ appsettings.Production.json |
| 依赖审计 | ✅ NuGet 全解决方案无已知漏洞；npm 全量审计 0 漏洞；审计服务失败会阻断 |
| 生产脚本/启动门禁 | ✅ 三镜像发布/滚动回滚/蓝绿切换、环境校验、独立凭据与 TLS/MQTT 证书 fail-closed 检查、证书生命周期指标/告警契约、应用种子账户启动校验、边缘网关租户/持久化路径校验、Production runtime smoke 与默认全量 E2E 门禁、备份、恢复与 CI 契约行为测试通过；本机隔离 smoke 已用当前提交本地构建的三镜像和固定 digest 基础层通过，固定 digest 的 CI runner 仍需按发布流水线验证 |
