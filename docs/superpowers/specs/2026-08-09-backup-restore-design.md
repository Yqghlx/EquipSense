...
- 新增 `docker/restore.sh`，支持 PostgreSQL SQL 压缩备份、工单附件归档和可选 Redis RDB。
- 默认执行 dry-run；任何会修改数据库、附件卷或 Redis 的动作都必须显式传入 `--confirm`。
- 恢复前校验备份文件存在、权限不过宽、gzip/tar 可读、Redis RDB 文件头有效，拒绝附件归档中的路径穿越和符号链接，并校验目标 Compose 配置和容器状态满足要求。
- 数据库恢复在停后端后执行，先终止目标数据库连接并重建数据库，再以 `ON_ERROR_STOP` 和单事务导入 SQL，覆盖 TimescaleDB 内部 schema，避免将备份内容追加到旧数据。
...
- `bash -n` 检查脚本语法。
- 使用临时目录和假的 `docker` 可执行文件覆盖：默认 dry-run 不调用 Docker、损坏 gzip/tar/RDB 和不安全附件归档被拒绝、确认路径使用一次性任务容器清理已停止后端共享的附件卷并处理 Redis AOF/权限、参数缺失失败关闭。
- 生产脚本回归测试纳入 CI。
