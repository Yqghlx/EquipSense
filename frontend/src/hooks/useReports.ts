...
export async function downloadOperationsReport(startDate?: string, endDate?: string) {
  ...
  downloadBlob(response.data as Blob, `operations_report_${Date.now()}.csv`);
}
...
export async function downloadCurrentMonthReport() {
  ...
  const now = new Date();
  downloadBlob(response.data as Blob, `operations_report_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}.csv`);
}
