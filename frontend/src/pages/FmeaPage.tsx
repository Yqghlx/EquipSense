...
import { AlertTriangle, Search, Trash2, Power, PowerOff } from 'lucide-react';
              ...
              {isLoading ? (
                ...
                data.items.map((entry) => (
                  <TableRow key={entry.id}>
                    ...
                    <TableCell className="text-center"><Badge className={getRpnColor(entry.rpn)}>{entry.rpn}</Badge></TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        {perm.canEdit && (
                          <Button variant="ghost" size="icon" onClick={() => toggleMutation.mutate(entry.id)}>
                            {entry.isEnabled ? <PowerOff className="h-4 w-4" /> : <Power className="h-4 w-4" />}
                          </Button>
                        )}
                        {perm.canDelete && (
                          <Button variant="ghost" size="icon" onClick={() => { if (confirm(t('fmea.confirmDelete'))) deleteMutation.mutate(entry.id); }}>
