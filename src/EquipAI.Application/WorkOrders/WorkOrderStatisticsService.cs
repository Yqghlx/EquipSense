...
public class WorkOrderStatisticsService
{
    ...
    {
        var startDate = DateTime.UtcNow.Date.AddDays(-periodDays + 1);
        ...
        // 新建趋势
        result.CreatedTrend = BuildTrend(startDate, workOrders
            .GroupBy(w => w.CreatedAt.Date)
            .ToDictionary(g => g.Key, g => g.Count()));

        // 完成趋势
        var completedOrders = workOrders.Where(w => w.CompletedAt.HasValue).ToList();
        result.CompletedTrend = BuildTrend(startDate, completedOrders
            .GroupBy(w => w.CompletedAt!.Value.Date)
            .ToDictionary(g => g.Key, g => g.Count()));
    ...
    {
        var days = (int)(DateTime.UtcNow.Date - startDate).Days + 1;
