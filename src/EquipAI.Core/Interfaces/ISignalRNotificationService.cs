...
/// <summary>
/// SignalR 实时推送服务接口
/// 定义告警、工单、知识规则和遥测数据的实时推送能力。
/// 业务通知按租户内活动用户及其渠道偏好定向，遥测数据使用租户组广播。
/// </summary>
public interface ISignalRNotificationService
{
    /// <summary>
    /// 推送告警触发事件到符合偏好的租户内用户组
    /// </summary>
    ...
    /// <summary>
    /// 推送告警解决事件到符合偏好的租户内用户组
    /// </summary>
    Task SendAlertResolvedAsync(Guid tenantId, Guid alertId,
        CancellationToken cancellationToken = default);

    /// <summary>
    /// 推送工单创建事件到符合偏好的租户内用户组
    /// </summary>
    Task SendWorkOrderCreatedAsync(Guid tenantId, Guid workOrderId, Guid deviceId,
        string title, string priority, CancellationToken cancellationToken = default);

    /// <summary>
    /// 推送工单状态变更事件到符合偏好的租户内用户组
    /// </summary>
    Task SendWorkOrderStatusChangedAsync(Guid tenantId, Guid workOrderId,
        string oldStatus, string newStatus, CancellationToken cancellationToken = default);

    /// <summary>
    /// 推送工单 AI 根因分析完成事件到符合偏好的租户内用户组
    ///
